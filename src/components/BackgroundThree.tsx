...
export const BackgroundThree: React.FC<BackgroundThreeProps> = ({ type = "hero" }) => {
  ...
  useEffect(() => {
    ...
    if (type === "hero") {
      // Hero Torus Knot (Wireframe, Accent gradient colors)
      const geometry = new THREE.TorusKnotGeometry(6, 1.6, 120, 16);
    ...
    const animate = () => {
      ...
      // Rotate Wireframe Toruses
      if (torusKnot) {
        torusKnot.rotation.y = elapsedTime * 0.05;
        torusKnot.rotation.x = elapsedTime * 0.02;
      }
    ...
    return () => {
      ...
      if (torusKnot) {
        ...
        (torusKnot.material as THREE.Material).dispose();
      }
